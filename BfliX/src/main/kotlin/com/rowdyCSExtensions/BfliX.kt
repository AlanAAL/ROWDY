...
class BfliX(val plugin: BfliXPlugin) : MainAPI() {
    override var mainUrl = BfliX.mainUrl
    override var name = BfliX.name
    ...
    override suspend fun loadLinks(
        ...
        serversRes?.select("span.server")?.forEach { server ->
            ...
            BfliXExtractor().getUrl(url, sName, subtitleCallback, callback)
        }
        return true
    }

    private suspend fun apiCall(prefix: String, data: String): Document? {
        val vrf = BfliXUtils.vrfEncrypt(data)
    ...
    private suspend fun getServerUrl(data: String): String {
        ...
        if (res?.status == 200) {
            return BfliXUtils.vrfDecrypt(res.result.url)
        }
        return ""
    }

    private suspend fun getSubtitles(episodeId: String): Array<BfliXSubtitle> {
        val res =
                app.get("$mainUrl/ajax/episode/subtitles/$episodeId")
                        .parsedSafe<Array<BfliXSubtitle>>()
